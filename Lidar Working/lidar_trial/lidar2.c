...
// single is held 0.3s (real time, not frame count) before commiting it,
// so a double isn't seen as single first
...
/* ---- tuning ----
 * All timing here is real elapsed milliseconds (via a monotonic clock
 * counter), NOT frame counts and NOT usleep. Detection itself (reading
 * serial lines, classify_frame) is never blocked or slowed down --
 * only the single-hold window and the visual redraw are time-gated.
 */
#define GAP_MIN                3
#define DOUBLE_VOTES_NEEDED    3
#define STABLE_FRAMES          3    // consecutive matching frames before confirming any state

#define SINGLE_HOLD_MS       300    // 0.3s real-time hold before a lone single commits
#define DISPLAY_REDRAW_MS     10    // how often the terminal grid / bitmap window repaints
...
// current time in whole milliseconds, monotonic clock
                ...
                !wait_key( 1, &pressed_key))
        {
              ...
              (strlen(serial_line) > 3))
          {
            ...
            if (r == (LIDAR_HEIGHT - 1))   // gets whole frame now
            {
              ...
              // real-time hold check -- 0.3s of elapsed wall clock time,
              // not a frame count, so it's correct regardless of the
              // sensor's actual frame rate
              if (holding_single)
              {
                long elapsed_ms = current_time_ms() - hold_start_ms;
                if (elapsed_ms >= SINGLE_HOLD_MS)   // time up, still single
                {
                  holding_single = false;
                  set_leds( io, 1 );
                  commit_single( hold_prev_state, log_file );
              ...
              // ==========================================
              // VISUAL REDRAW -- throttled by a monotonic
              // millisecond counter, NOT usleep. Never blocks
              // the detection logic above.
              // ==========================================
              long now_ms = current_time_ms();
              bool should_redraw = (!have_displayed_once) ||
                                   ((now_ms - last_display_ms) >= DISPLAY_REDRAW_MS);
