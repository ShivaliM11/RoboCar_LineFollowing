  ...
  while (!g_quit)
  {
    ...
    if (!(FIFO_EMPTY(param->key_fifo)))
    {
      ...
      if (cmd.command == 115)  // 's' = stop, always works
      {
        turn_ticks = 0;
        g_tracing = false;
        g_motor_stop = true;  // stop LineTrace immediately
      ...
      switch ((g_mode << 8) | cmd.command)
      {
        ...
        case (2 << 8) | 119:
          g_motor_stop = false;   // m2 w start tracing
        ...
        case (1 << 8) | 119:
          g_motor_stop = false;   // m1 w forward
          turn_ticks = 0;
        ...
        case (1 << 8) | 120:
          g_motor_stop = false;   // m1 x backward - mirror of forward case
          turn_ticks = 0;
          if (cur_dir == 1)
          { for (step = RAMP_STEPS; step >= 0; step--)
            { ramp = cur_speed * step / RAMP_STEPS;
...
void *MotorThread(void * arg)
{
  ...
  while (!g_quit)
  {
    if (!(FIFO_EMPTY(param->fifo)))
    {
      ...
      switch (cmd.command)
      {
        case 'f': if (!g_motor_stop) { GPIO_CLR(param->gpio, param->pin1); GPIO_SET(param->gpio, param->pin2); } break;
