...
void motor_backward(struct io_peripherals *io) {
  GPIO_CLR(io->gpio, 05);   //forward-spin left motor 
  GPIO_SET(io->gpio, 06);   //  1,0=GPIO05,GPIO06
...
void *ir_thread(void *arg)
{
  ...
  while(1)
  {
    ...
    int center_sensor_val = (io->gpio->GPLEV0 >> 21) & 1; // shift the bit and AND with 1
    ...
    else if ((left_sensor_val == 0) && (right_sensor_val == 0) && (center_sensor_val == 0))
    {
      ...
      if ((shared_data->end_of_row_count == 0) && (shared_data->all_white_count >= 2) && ((left_side_sensor_val == 1) || (right_side_sensor_val == 1)) && (shared_data->last_turn_dir == DIR_NONE))  // if all the front sensors are white for more than 50ms -> INTERSECTION
      {
...
int main(void)
{
  ...
  if (io != NULL)
  {
    ...
    io->gpio->GPFSEL2.field.FSEL1 = GPFSEL_INPUT; // GPIO26
